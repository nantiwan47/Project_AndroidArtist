...
public class MyAdapter extends RecyclerView.Adapter<MyAdapter.ViewHolder>{
    ...
    List<Artist> values;
    Context context;    // หมายถึง MainActivity context ที่ส่งมาตอนเรียกใช้ MyAdapter

    // constructor ของ MyAdapter ทำหน้าที่นำค่าที่ได้รับมาตอนถูกสร้างที่ MainActivity มาเก็บไว้ที่ global variables
    public MyAdapter(List < Artist > values, Context context) {
        this.values = values;
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        final Artist artist = values.get(position);
        holder.txtHeader.setText(artist.getArtist_name());
        holder.txtFooter.setText("เดบิวต์ พ.ศ. " + artist.getArtist_debut());

        //ใช้ Glide ในการแสดงภาพบน ImageView  ซึ่งอาจจะใช้ Picasso แทนก็ได้
        Glide.with(context)
                .load(artist.getArtist_image())
                .into(holder.imgView);
